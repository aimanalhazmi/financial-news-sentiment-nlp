...
from src.utils import count_number_of_words, get_unique_words,create_reports_subfolder, print_sentiment_distribution
...
def plot_word_count_distribution_by_sentiment(df, text_length_col, sentiment_col, save=False, save_path=''):
    ...
    for sentiment in unique_sentiments:
        ...
        plt.hist(sentiment_subset[text_length_col], alpha=0.5, label=str(sentiment), bins=15)
    plt.legend()
...
def plot_data_distribution_by_sentiment(df, sentiment_col, save=False, save_path=''):
    ...
    counts.plot.bar(xlabel="Sentiment", ylabel="Count", color=colors, title='Data Distribution by Sentiment')
    if save and save_path:
        plt.savefig(save_path, bbox_inches='tight')
    #    if not save:
    #        plt.show()
...
def plot_unique_words_by_sentiment(df, text_column, sentiment_col, save=False, save_path=''):
    ...
    unique_counts = {sentiment: len(get_unique_words(df[df[sentiment_col] == sentiment][text_column])) for sentiment in sentiments}
    ...
    legend_elements = [Patch(facecolor=color_map.get(sentiment, 'gray'), label=f'{sentiment}') for sentiment in
                       unique_counts.keys()]
...
def plot_most_common_words_by_sentiment(df, text_column, sentiment_col, top_n=30, save=False, save_path=''):
    ...
    for i, sentiment in enumerate(sentiments):
        ...
        axes[i].set_title(f"Top {top_n} Words in {sentiment.capitalize()} Financial News", fontsize=24)
...
def generate_word_cloud_by_sentiment(df, text_column, sentiment_col, save=False, save_path=''):
    """
...
def generate_dataset_insights(
    ...
    df = count_number_of_words(df=df, column_to_count=text_column, count_to='word_count')
    ...
    plot_data_distribution_by_sentiment(
        df, sentiment_col=sentiment_col,
        save=save, save_path=f"{save_dir}/sentiment_distribution.png"
    )

    plot_word_count_distribution_by_sentiment(
        df, text_length_col='word_count', sentiment_col=sentiment_col,
        save=save, save_path=f"{save_dir}/word_count_distribution.png"
    )

    plot_avg_word_count_by_sentiment(
        df, text_length_col='word_count', sentiment_col=sentiment_col,
        save=save, save_path=f"{save_dir}/avg_word_count.png"
    )

    plot_unique_words_by_sentiment(
        df, text_column=text_column, sentiment_col=sentiment_col,
        save=save, save_path=f"{save_dir}/unique_words.png"
    )

    plot_most_common_words_by_sentiment(
        df, text_column=text_column, sentiment_col=sentiment_col,
        save=save, save_path=f"{save_dir}/most_common_words.png"
    )

    generate_word_cloud_by_sentiment(
        df, text_column=text_column, sentiment_col=sentiment_col,
        save=save, save_path=f"{save_dir}/word_cloud.png"
    )
